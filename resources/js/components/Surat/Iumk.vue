<template>
  <h4 class="fw-bold py-3 mb-4">SURAT IZIN USAHA MIKRO DAN KECIL</h4>
  <div class="row">
    <div class="col-lg-12">
      <div class="card">
  ...
  <modal ref="modal" :size="'lg'" @on-hide="reset" :title="(letter.id == '' ? 'Tambah' : 'Ubah') + ' Surat IUMK'"
    :close_button="true">
    <form @submit.prevent="saveLetter">
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" placeholder="PT. XXX XXX XXX" v-model="letter.company_name" />
      </div>
      <div class="mb-3">
        <label class="form-label">Alamat Perusahaan</label>
        <textarea class="form-control" placeholder="JL. XXX XXX XXX" v-model="letter.company_address"></textarea>
      </div>
      <div class="row">
        ...
        <div class="mb-3 col-lg-6">
          ...
          <input type="number" class="form-control" placeholder="15.000.000" v-model="letter.capital" />
        </div>
      </div>
      <div class="mb-3">
  ...
  <modal ref="modal_verification" :size="'lg'" @on-hide="reset" :title="'Verifikasi Surat IUMK'" :close_button="true">
  ...
  <modal ref="modal_form_official" :title="'Tambah Penandatangan'" :button_close="true">
    <official-form @on-save="$refs.modal_form_official.hide(), spinner.show()" @finish="savedOfficial"></official-form>
  </modal>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      url: `${origin}/backend/iumk-letters`,
      modal: null,
  ...
  methods: {
    ...
    async saveLetter() {
      ...
      try {
        this.spinner.show();
        payload.append('company_name', this.letter.company_name);
        payload.append('company_address', this.letter.company_address);
        payload.append('business', this.letter.business);
        payload.append('capital', this.letter.capital);
