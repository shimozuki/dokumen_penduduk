<template>
  <h4 class="fw-bold py-3 mb-4">
    SURAT KETERANGAN TIDAK MAMPU UNTUK SEKOLAH ATAU INSTANSI TERKAIT
  </h4>
  <div class="row">
    <div class="col-lg-12">
      <div class="card">
        ...
        <div class="card-body">
          <custom-table ref="table" :thead="head" :url="url" v-slot="data">
            ...
            <td>{{ data.reference_number_desa }}</td>
  ...
  <modal ref="modal" :size="'xl'" @on-hide="reset"
    :title="(letter.id == '' ? 'Tambah' : 'Ubah') + ' Surat SKTM Untuk Sekolah/Instansi'" :close_button="true">
      ...
      <div class="row">
        <div class="col-lg-6 mb-3">
          ...
          <default-file-upload v-model="surat_pengantar_file" v-model:preview="letter.surat_pengantar"
            :accepted_file_type="['jpg', 'jpeg', 'png', 'pdf']" />
        </div>
        <div class="col-lg-6 mb-3">
...
">
      ...
      <div class="row">
        <div class="mb-3 col-lg-6">
          ...
          <input type="text" class="form-control" placeholder="Nama Ayah" v-model="letter.father" />
        </div>
        <div class="mb-3 col-lg-6">
          <label class="form-label">Nama Ibu</label>
          <input type="text" class="form-control" placeholder="Nama Ibu" v-model="letter.mother" />
        </div>
      </div>
      <div class="mb-3">
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" placeholder="Nama Sekolah/Institusi" v-model="letter.institution" />
      </div>
      <div class="mb-3">
...
">
  ...
  <modal ref="modal_verification" :size="'lg'" @on-hide="reset" :title="'Verifikasi Surat SKTM Untuk Sekolah/Instansi'"
    :close_button="true">
        ...
        <form method="POST" @submit.prevent="savePenandatanganKecamatan">
          ...
          <div class="mb-3">
            ...
            <search-input placeholder="Cari Aparat Desa ..." ref="search_input_penandatangan_kecamatan"
              ...
              <template #default_first>
                <p class="text-center text-primary m-0" style="cursor: pointer"
                  @click="modal_form_penandatangan_kecamatan.show(), ($refs.search_input_penandatangan_kecamatan.focus = false)">
                  + Tambah Data Aparat Desa
                </p>
              </template>
            </search-input>
...
export default {
  ...
  data() {
    return {
      ...
      url: `${origin}/backend/sktm-sekolah-letters`,
      modal: null,
  ...
  methods: {
    ...
    async saveLetter() {
      ...
      try {
        ...
        letter.append('father', this.letter.father);
        letter.append('mother', this.letter.mother);
        letter.append('used_as', this.letter.used_as);
        letter.append('institution', this.letter.institution);
        ...
        if (!this.letter.resident) {
          throw "resident_is_null";
        }
        letter.append('resident_id', this.letter.resident.id);
