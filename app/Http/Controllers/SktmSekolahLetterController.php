...
use App\Http\Resources\SktmSekolahLetterCollection as ThisCollection;
use App\Models\SktmSekolahLetter as ThisLetter;
...
class SktmSekolahLetterController extends Controller
{
    ...
    {
        $letters = ThisLetter::with(['resident', 'penandatangan_kecamatan', 'penandatangan_desa'])->byUserId(auth()->id())
            ->where(function ($q) use ($request) {
                $q->where(DB::raw("CONCAT(prefix, sktm_sekolah_letters.order, suffix)"), 'like', '%' . $request->search . '%')
                    ->orWhere(DB::raw("CONCAT(prefix_desa, sktm_sekolah_letters.order_desa, suffix_desa)"), 'like', '%' . $request->search . '%')
    ...
    {
        $letter = ThisLetter::byUserId(auth()->id())->with(['resident', 'user', 'penandatangan_kecamatan', 'penandatangan_desa'])->where('id', $id)->first();

        if (!$letter) {
            return response()->json("Surat tidak ditemukan", 404);
        }

        if (!$letter->penandatangan_kecamatan_id) {
        ...
        $templateProcessor = new TemplateProcessor(storage_path("app/template_word/SKTM_SEKOLAH.docx"));
        ...
        $templateProcessor->setValue('nik', $letter->resident->nik);
        $templateProcessor->setValue('nama', $letter->resident->name);
        $templateProcessor->setValue('ttl', $letter->resident->place_of_birth . ", " . Carbon::parse($letter->resident->date_of_birth)->translatedFormat('j F Y'));
        $templateProcessor->setValue('jenis_kelamin', $letter->resident->gender);
        $templateProcessor->setValue('agama', $letter->resident->religion);
        $templateProcessor->setValue('pekerjaan', $letter->resident->profession);
        $templateProcessor->setValue('nama_ayah', $letter->father);
        $templateProcessor->setValue('nama_ibu', $letter->mother);
        $templateProcessor->setValue('alamat', $letter->resident->address);
        ...
        $templateProcessor->setValue('institusi', $letter->institution);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $new_letter->father = $request->father;
            $new_letter->mother = $request->mother;
            $new_letter->used_as = $request->used_as;
            $new_letter->institution = $request->institution;
            ...
            $signature_desa = 'penandatangan_desa_sktm_sekolah' . Str::uuid() . '.png';
            ...
            $message = "Notification \n-----------------------\n" . "Meminta Verifikasi Surat Keterangan Tidak Mampu Untuk Sekolah/Instansi\nNama Instansi : " . auth()->user()->name . "\nNomor Surat : " . $new_letter->get_reference_number_desa() . "\nLink Surat : http://" . $host . "/sktm-sekolah?search=" . $new_letter->get_reference_number();

            Whatsapp::send(auth()->user()->parent->whatsapp_number, $message);

            return response()->json("Surat SKTM untuk sekolah berhasil dibuat", 201);
        } catch (Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if (!$letter || !$letter->can_modified()) {
                return response()->json("Gagal memperbarui data. Surat SKTM untuk sekolah tidak ditemukan.", 404);
            }
            // update surat
            ...
            $update_letter->father = $request->father;
            $update_letter->mother = $request->mother;
            $update_letter->used_as = $request->used_as;
            $update_letter->institution = $request->institution;
            ...
            $message = "Notification \n-----------------------\n" . "Meminta Verifikasi Surat Keterangan Tidak Mampu Untuk Sekolah/Instansi\nNama Instansi : " . auth()->user()->name . "\nNomor Surat : " . $update_letter->get_reference_number_desa() . "\nLink Surat : http://" . $host . "/sktm-sekolah?search=" . $update_letter->get_reference_number();

            Whatsapp::send(auth()->user()->parent->whatsapp_number, $message);

            return response()->json("Surat SKTM untuk sekolah berhasil diubah", 200);
        } catch (Exception $e) {
            ...
            if ($e->getMessage() == 'resident_not_found') {
                ...
                return response()->json("Surat SKTM untuk sekolah berhasil diubah, tapi notifikasi whatsapp gagal dikirim", 200);
            } else {
                return response()->json($e->getMessage() . " In line " . $e->getLine(), 500);
            }
        }
    ...
    public function verification(Request $request, $id)
    {
        ...
        if (!$letter || !$letter->can_verified()) {
            return response()->json("Surat SKTM untuk sekolah tidak ditemukan", 404);
        }

        if ($letter->is_verified()) {
            return response()->json("Surat SKTM untuk sekolah telah di verifikasi", 200);
        }
        ...
        $message = "Notification \n-----------------------\n" . "Surat Keterangan Tidak Mampu Untuk Sekolah/Instansi Telah Di Verifikasi Oleh\nNama Instansi : " . auth()->user()->name . "\nNomor Surat : " . $letter->get_reference_number_desa() . "\nLink Surat : http://" . $host . "/sktm-sekolah?search=" . $letter->get_reference_number();

        Whatsapp::send($letter->user->whatsapp_number, $message);

        return response()->json("Surat SKTM untuk sekolah berhasil di verifikasi", 200);
    }
    ...
    public function upload_file_arsip(Request $request, $id)
    {
        ...
        try {
            ...
            if (!$letter || !$letter->can_modified() || !$letter->is_verified()) {
                return response()->json("Gagal upload file arsip data. Surat SKTM untuk sekolah tidak ditemukan.", 404);
            }
            // update surat
            $update_letter = $letter;
            $file_arsip_old = null;
            ...
            return response()->json("File arsip surat SKTM untuk sekolah berhasil diupload", 200);
        } catch (Exception $e) {
            //Kembalikan segala transaction ke database
            DB::rollBack();
    ...
    public function destroy($id)
    {
        $letter = ThisLetter::byUserId(auth()->id())->where('id', $id)->first();
        if (!$letter || !$letter->can_modified()) {
            return response()->json("Gagal menghapus data. Surat SKTM untuk sekolah tidak ditemukan.", 404);
        }
        ...
        try {
            ...
            return response()->json("Surat SKTM untuk sekolah berhasil dihapus", 200);
        } catch (Exception $e) {
            DB::rollBack();
            return response()->json($e->getMessage() . " " . $e->getLine());
            return response()->json("Surat SKTM untuk sekolah gagal dihapus", 500);
        }
    }
}
